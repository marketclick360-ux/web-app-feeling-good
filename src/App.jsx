    ...
    <>
      ...
      {tab === 'todos' && (
        <div className="todo-tab">
          ...
          <section className="card"><h3 className="section-heading notes-heading">{"270dfe0f"} Journal<button className={`copy-btn ${copiedId === 'todoJournal' ? 'copied' : ''}`} onClick={() => copyToClipboard(journalText, 'todoJournal')} title="Copy journal">{copiedId === 'todoJournal' ? '2705' : 'd83ddccb'}</button></h3><RichNoteEditor value={journalText} onChange={setJournalText} placeholder="Write your thoughts, reflections, and experiences..." minHeight={200} /></section>
        </div>
      )}
